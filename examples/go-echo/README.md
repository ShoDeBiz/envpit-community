...
A minimal [Echo](https://github.com/labstack/echo) web service that pulls its config from a
real EnvPit server through the **published** Go SDK
...
`os.Getenv("SOME_KEY")` code (Echo's own config idiom; Echo has no `@Value`/`application.yml`
